...
static SOCKET epoll__get_peer_socket(epoll_port_data_t* port_data, WSAPROTOCOL_INFOW* protocol_info);
static SOCKET epoll__create_peer_socket(HANDLE iocp, WSAPROTOCOL_INFOW* protocol_info);
static int epoll__compare_sock_data(epoll_sock_data_t* a, epoll_sock_data_t* b);
static int epoll__submit_poll_op(epoll_port_data_t* port_data, epoll_sock_data_t* sock_data);
...
RB_GENERATE_STATIC(epoll_sock_data_tree, epoll_sock_data_s, tree_entry, epoll__compare_sock_data)
...
int epoll_ctl(epoll_t port_handle, int op, SOCKET sock, struct epoll_event* ev)
{
  ...
  switch (op)
  {
    case EPOLL_CTL_ADD:
    {
      ...
      if (0 != getsockopt(base_sock, SOL_SOCKET, SO_PROTOCOL_INFOW, (char*) &protocol_info, &len))
        return -1;

      if (INVALID_SOCKET == (peer_sock = epoll__get_peer_socket(port_data, &protocol_info)))
          return -1;

      if (!(sock_data = malloc(sizeof *sock_data)))
      {
        SetLastError(ERROR_OUTOFMEMORY);
        return -1;
      }
      ...
      if (NULL != RB_INSERT(epoll_sock_data_tree, &port_data->sock_data_tree, sock_data))
      {
    ...
    case EPOLL_CTL_MOD:
    {
      ...
      if (!(sock_data = RB_FIND(epoll_sock_data_tree, &port_data->sock_data_tree, &lookup)))
      {
      ...
      {
        if (!sock_data->free_op)
        {
          sock_data->free_op = malloc(sizeof *sock_data->free_op);
...
int epoll_wait(epoll_t port_handle, struct epoll_event* events, int maxevents, int timeout)
{
  ...
  do
  {
    ...
    while (port_data->attn_list != NULL)
    {
      ...
      if (sock_data->registered_events & EPOLL__EVENT_MASK & ~sock_data->submitted_events)
      {
    ...
    result = GetQueuedCompletionStatusEx(port_data->iocp, entries, max_entries, &count, gqcs_timeout, FALSE);
...
int epoll_close(epoll_t port_handle)
{
  ...
  while (port_data->pending_ops_count > 0)
  {
    ...
    result = GetQueuedCompletionStatusEx(port_data->iocp, entries, ARRAY_COUNT(entries), &count, INFINITE, FALSE);
...
int epoll__initialize()
{
  ...
  if (!(pNtDeviceIoControlFile = (PNTDEVICEIOCONTROLFILE)GetProcAddress(ntdll, "NtDeviceIoControlFile")))
    return -1;
...
SOCKET epoll__get_peer_socket(epoll_port_data_t* port_data, WSAPROTOCOL_INFOW* protocol_info)
{
  int i, index = -1;
  ...
  /* protocol. */
  if (!port_data->peer_sockets[index])
    port_data->peer_sockets[index] = epoll__create_peer_socket(port_data->iocp, protocol_info);

  return port_data->peer_sockets[index];
}


SOCKET epoll__create_peer_socket(HANDLE iocp, WSAPROTOCOL_INFOW* protocol_info)
{
  SOCKET sock = 0;
...
int epoll__submit_poll_op(epoll_port_data_t* port_data, epoll_sock_data_t* sock_data)
{
  ...
  if (0 != epoll__afd_poll(sock_data->peer_sock, &op->poll_info, &op->overlapped))
  {
...
int epoll__afd_poll(SOCKET socket, AFD_POLL_INFO* info, OVERLAPPED* overlapped)
{
  ...
  status = pNtDeviceIoControlFile((HANDLE) socket, event, NULL, apc_context, iosb_ptr, IOCTL_AFD_POLL, info, sizeof *info, info, sizeof *info);
